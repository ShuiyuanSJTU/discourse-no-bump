...
require_relative "lib/discourse_no_bump/engine.rb"
...
after_initialize do
  ...
  Discourse::Application.routes.append { mount ::DiscourseNoBump::Engine, at: "/no_bump" }
  ...
  module OverrideNoBumpWhenMove
    def update_last_post_stats
      ...
      if post && post_ids.include?(post.id)
        ...
        attrs[:last_post_user_id] = post.user_id
        # modify here
        attrs[:bumped_at] = Time.now if !destination_topic.custom_fields["no_bump"]
        attrs[:updated_at] = Time.now
  ...
  ::PostCreator.prepend OverrideNoBumpWhenCreate
  ::PostRevisor.prepend OverrideNoBumpWhenRevise
  ::PostMover.prepend OverrideNoBumpWhenMove
  ::Topic.prepend OverrideTopic
  ...
  TopicQuery.add_custom_filter(:no_bump) do |results, topic_query|
    if topic_query.options[:no_bump] && topic_query.options[:no_bump] == "true"
      results =
        ...
        ).where("topic_custom_fields.name = 'no_bump' AND topic_custom_fields.value = 't'")
    end
    results
  end
  ...
  register_modifier(:topic_query_create_list_topics) do |topics, options, topic_query|
    ...
    SQL
    topics = topics.where(closed: false) if SiteSetting.hide_closed_topics_in_hot_topics
    topics
  end
end
